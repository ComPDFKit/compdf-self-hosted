 ...
 * apiClient, without async task flow or API-key headers.
 *
 * State machine:
...
/**
 * Build the multipart FormData per the middleware contract and POST it as a
 * blob stream. Field names confirmed against the server controllers:
 *   - /pdf/merge            → FilesInterceptor('files')           → 'files'
 *   - /pdf/insert-from-pdf  → FileFieldsInterceptor file+insertFile → 'file' + 'insertFile'
 *   - /pdf/watermark/add    → FileFieldsInterceptor file+imageFile  → 'file' + 'imageFile'
 *   - other /pdf/{op}       → FileInterceptor('file')              → 'file'
 *   - /process/{source}/{target} → FileInterceptor('file'); middleware maps the
...
function apiErrorKey(error: Awaited<ReturnType<typeof toApiError>>): string | null {
  ...
  if (typeof error.code === 'number' && API_NUMERIC_CODE_KEYS[error.code]) {
    return API_NUMERIC_CODE_KEYS[error.code];
  }
  if (typeof error.code === 'string') {
    return API_ERROR_CODE_KEYS[error.code] ?? null;
  }
  return null;
}
  ...
  >
    <template v-if="file && file.status === 'await'">
      ...
      <div class="name">
        ...
        <DeleteIcon class="name-delete" @click="handleDelete" />
      </div>

      <div class="await-actions">
        <button class="select-more" @click="handleClick(canSelectMultiple ? 'add' : 'replace')">
      ...
      <button v-if="!file.percentage" class="btn select-btn action-btn" @click="handleConvert(true)">
        {{ convertButtonLabel }}
      </button>
    ...
    >
      <template v-if="!file">
        ...
        <button class="btn select-btn" @click="handleClick()">
      ...
      <template v-else-if="file.status === 'selectType'">
        ...
        <button class="btn convert-btn" @click="handleConvert(false)">
