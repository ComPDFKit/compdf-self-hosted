/**
 * Parameter schema + request builder (pure logic, no UI framework, no network).
 *
 * Maps the shared upload `parameter` + `password` state onto the server's
 * two request shapes:
 *
 *   - PDF routes (`/api/v1/process/pdf/{op}`): `extractRequest` parses `body.request`
 ...
 *     `field: 'request'`.
 *   - Conversion routes (`/api/v1/process/{source}/{target}`):
...
export const COMPRESS_PRESET_FLAGS: Record<'low' | 'medium' | 'high', string[]> = {
...
function buildPdfEditPayload(input: BuildRequestInput): { payload: Record<string, unknown>; extraFiles?: File[]; imageFile?: File } {
  ...
  if (toType === 'delete') { return { payload: { pageRanges, outputFileName } }; }
  if (toType === 'rotate') { return { payload: { pageRanges, angle: Number(parameter.rotateAngle) || 90, outputFileName } }; }
  if (toType === 'compress') {
    const result: Record<string, unknown> = { outputFileName };
    const q = parameter.compressQuality;
    if (q === 'low' || q === 'medium' || q === 'high') {
      result.profile = 'aggressive';
      result.imageQuality = q === 'low' ? 30 : q === 'medium' ? 60 : 80;
    }
    else if (q === 'custom') {
  ...
  if (toType === 'addWatermark') {
    const result: Record<string, unknown> = {
      ...
      pages: pageRanges, fullScreen: parameter.watermarkFullScreen, outputFileName,
    };
  ...
  if (toType === 'removeWatermark') { return { payload: { outputFileName, pages: pageRanges, mode: 'tagged' } }; }
  if (toType === 'encrypt') {
    const result: Record<string, unknown> = { outputFileName };
