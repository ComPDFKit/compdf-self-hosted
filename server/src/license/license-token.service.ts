/**
 * LicenseTokenService — READ-ONLY, DISPLAY-ONLY.
 *
 * ★ Red line (architecture §5/§8.1): the server NEVER interprets, validates,
 * or enforces license limits. This service only (a) hands the raw signed JWT to
 * the passthrough interceptor so clients can attach `X-ComPDF-License`, and
 * (b) decodes the payload for DISPLAY in the ComPDF Web — without verifying the
 * signature (the server has no public key; the closed-source app verifies).
 *
...
export class LicenseTokenService {
  ...
  /**
   * The raw signed JWT to attach as `X-ComPDF-License`.
